...
**Портативная сборка для удаления водяных знаков и объектов с видео и фото под Windows. Установка в один клик, работа офлайн.**
...
- **Масштабирование** изображения до 900% для точной работы
- Поддержка **GPU** (NVIDIA CUDA) и **CPU**
- Работа с файлами, содержащими **русские буквы** в именах
- Drug-and-Drop загрузка файлов для обработки
...
- Панорамы и фото 30+ мегапикселей больше не подвешивают приложение
- Автоматический выбор стратегии: небольшие изображения — целиком, крупные — обработка по областям маски с сохранением полного разрешения на выходе
...
1. Нажмите **Load Media** и выберите видеофайл
2. Дождитесь извлечения кадров
3. На первом кадре **кистью** выделите водяной знак (розовая область)
   - Настройте размер кисти ползунком **Brush Size**
   - Для исправления используйте **ПКМ** (ластик)
4. Переключайтесь между кадрами с помощью **Previous / Next** или таймлайна
5. Нажмите **Remove** — программа обработает все кадры видео
6. Сохраните результат в нужном формате

### Удаление объектов с фотографии

1. Нажмите **Load Media** и выберите изображение
2. **Кистью** выделите объект, который нужно удалить
3. Нажмите **Remove**
4. Сохраните результат (PNG или JPG)

### Описание элементов интерфейса

| Элемент | Назначение |
|---------|------------|
| **Load Media** | Загрузить видео или изображение |
| **Clear** | Очистить текущую маску |
| **Remove** | Обработать изображение / все кадры видео |
| **Previous / Next** | Переключение кадров видео |
| **Play / Pause** | Воспроизведение видео |
| **Timeline** | Перемещение по кадрам видео |
| **Thumbnails** | Миниатюры кадров для быстрого перехода |
| **Brush Size** | Размер кисти (3–200 пикселей) |
| **Zoom − / +** | Уменьшить / увеличить масштаб |
| **× (сброс зума)** | Вернуть масштаб «по размеру окна» |
| **Zoom %** | Текущий масштаб в процентах |
...
| **Ctrl + Z** | Отменить последний штрих |
| **Ctrl + Shift + Z** | Вернуть отменённый штрих |
| **ЛКМ + движение** | Рисование маски |
| **ПКМ + движение** | Ластик (стирание маски) |
...
├── ffmpeg/                # FFmpeg для работы с видео
└── cache/                 # Кэш пакетов
```
...
- Запустите установку повторно: `WatermarkRemover.bat` → **2. Install / Re-install**
