...
echo [STEP] Installing PyTorch (compatible with NumPy 2.x)...
if "%HAS_GPU%"=="1" (
    echo [INFO] Installing torch+cu128 for NVIDIA GPU...
    uv pip install --force-reinstall torch torchvision --index-url https://download.pytorch.org/whl/cu128 --python "%VENV%\python.exe"
) else (
    echo [INFO] Installing CPU torch...
...
if "%HAS_GPU%"=="1" (
    echo [INFO] Re-installing PyTorch+cu128...
    uv pip install --force-reinstall torch torchvision --index-url https://download.pytorch.org/whl/cu128 --python "%VENV%\python.exe"
) else (
    echo [INFO] Re-installing PyTorch CPU...
    uv pip install --force-reinstall torch torchvision --index-url https://download.pytorch.org/whl/cpu --python "%VENV%\python.exe"
)
